...
For an email action, \`action:verify\` returns an approval-bound \`emailDelivery\`. Use
\`fromAddress\` only to select the authenticated connector account and set the complete RFC From
header to the exact display-name-bearing \`fromHeader\`. Send only its exact
\`multipart/alternative\` payload, recipients, and attachments. Read the delivered MIME and actual
From header back through the connector, report that header as \`deliveredFromHeader\`, then provide
the exact \`emailDeliveryReadback\` through a file-backed completion result. Tend rejects a bare or
wrong sender identity, text-only MIME, or any mismatch. Direct connector calls outside Tend remain
outside this gate.
...
For approved sends, use \`fromAddress\` only to select the authenticated connector account, set the
complete RFC From header to the exact display-name-bearing \`fromHeader\`, and use only the
\`multipart/alternative\` payload returned by \`action:verify\`. After sending, read the delivered
MIME and actual From header back and include that header as \`deliveredFromHeader\` in the matching
file-backed completion receipt. Tend must reject a bare or wrong sender identity, text-only
delivery, and any recipient, body, attachment, or approval-digest mismatch. This gate covers Tend
completion, not direct connector calls made outside Tend.
