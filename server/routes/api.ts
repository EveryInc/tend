...
import type { EmailDeliveryReadbackInput, PostActionCompletion, VoiceTarget } from "../../shared/types";
...
export function apiRoutes(context: LocalRouteContext): Hono {
  ...
  app.post("/api/feeds/:feed/work/:work/reconcile-approved", async (c) => mutation(c, notify, async () => {
    const input = await body(c);
    const result = input.result && typeof input.result === "object" ? input.result as { response: string; done?: boolean; postAction?: PostActionCompletion; emailDeliveryReadback?: EmailDeliveryReadbackInput } : { response: "" };
