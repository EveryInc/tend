...
export function drainPrompt(feedId: string, threadId: string): string {
  return [
    ...
    "EMAIL SEND GATE: when action:verify returns emailDelivery, use fromAddress only to select the authenticated account and set the complete RFC From header to the exact display-name-bearing fromHeader. Send only its exact multipart/alternative payload, recipients, and attachments. Read the delivered MIME and actual From header back from the connector; include the latter as deliveredFromHeader in the exact emailDeliveryReadback passed to work:complete or work:reconcile-approved via --result-file. Tend rejects a bare or wrong sender identity, text-only MIME, or mismatching delivery receipt. Direct connector calls outside Tend remain outside this gate.",
