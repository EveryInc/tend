...
export function drainPrompt(feedId: string, threadId: string): string {
  return [
    ...
    "EMAIL SEND GATE: when action:verify returns emailDelivery, send only its exact multipart/alternative payload, fromAddress, recipients, and attachments. Read the delivered MIME back from the connector and include an exact emailDeliveryReadback in work:complete or work:reconcile-approved via --result-file. Tend rejects text-only or mismatching delivery receipts. Direct connector calls outside Tend remain outside this gate.",
