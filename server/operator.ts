...
export function formatWorkClaimOutput(feedId: string, work: WorkClaimResult, context: WorkClaimContext = {}): ClaimedWorkOutput | WorkClaimedByReport | IdleWorkHandshake {
  ...
  if (userAuthorization) {
    ...
    if (userAuthorization.sourceMailbox && userAuthorization.exactApprovedArtifact?.type === "editable_text") {
      operatorGuidance.emailDeliveryRule = "EMAIL SEND GATE: action:verify returns emailDelivery with the only payload authorized for this send. Use fromAddress only to select the authenticated connector account; set the message's complete RFC From header to fromHeader, including its display name. Pass the multipart/alternative payload, recipients, and attachments unchanged. Then read the delivered message back and use --result-file to report its actual From header as deliveredFromHeader plus recipients, MIME parts, and attachment metadata in emailDeliveryReadback; retain the verified version, fromAddress, fromHeader, approvalDigest, and payloadDigest, and add source=connector_readback, providerMessageId, and readAt. Never synthesize deliveredFromHeader or the rest of the readback from the proposed draft. Tend rejects a bare or wrong sender identity, text-only MIME, or any recipient, body, attachment, or approval-digest mismatch. A direct connector call outside Tend is outside this gate.";
    }
  }
