...
- `attention.db` stores local runtime metadata, active workspace feed membership, feed cards, feed audit events, and queued/claimed/completed work items.
...
- `data/` stores current feed artifacts that are still file-backed: prompts, source recipes, runs, checkpoints, routine action groups, sweep artifacts, and revisions.
