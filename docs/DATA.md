...
- `attention.db` stores local runtime metadata, active workspace feed membership, feed cards, routine action groups, feed audit events, and queued/claimed/completed work items.
...
- `data/` stores current feed artifacts that are still file-backed: prompts, source recipes, runs, checkpoints, sweep artifacts, and revisions.
...
The export command copies the local feed data directory. Active feed membership, cards, routine action groups, audit events, and work items are mirrored into `data/`, so feed visibility, UI artifacts, audit history, and queued agent work survive data-directory backup even though SQLite is now the local runtime authority for those records.
