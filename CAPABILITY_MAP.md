...
| Dismiss a card locally (no source change) | Press the card's `Dismiss card` control, with brief Undo | `card:dismiss-local` moves the card to done with no work item, no `action:verify`, and no connector call; reverse it with `card:return-to-review` |
| Clean up a card's source | Press the card's explicit cleanup action, often `Archive`, with brief Undo | Queue one digest-bound `default_cleanup`, call `action:verify`, drain it through Codex, and record the verified outcome |
