...
| Collect evidence | Render the resulting cards | `source:record-run` (judgments may name their `cardId`) writes immutable snapshots and, for claimed recollection work, holds the checkpoint on the run; `sweep:record-batch` records the judged batch; `sweep:status` lists what is still unpresented; `work:complete` commits the held checkpoints once every judgment is presented |
