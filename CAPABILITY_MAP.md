...
| Collect evidence | Render the resulting cards, or review a visible recovery card if presentation is interrupted | `source:record-run` stages immutable evidence and a checkpoint under a deterministic run ID; `sweep:record-batch` records stable kept-card obligations; the last complete `card:upsert` atomically commits cards, checkpoints, and the current batch; `sweep:status` reports missing cards |
