...
| Account for the complete Inbox | Review exactly one card per current Gmail Inbox thread | `sweep:record-inbox-page` durably records each provider response as it arrives; `sweep:finalize-inbox` reconstructs and verifies that immutable token/page ledger plus the complete thread/card mapping, then atomically activates the run, cards, removals, checkpoint, and coverage receipt |
...
| Collect evidence | Render the resulting cards | Inbox uses `sweep:record-inbox-page` then `sweep:finalize-inbox`; other feeds use `source:record-run` or `source:import-json-file` for immutable snapshots and `sweep:record-batch` for the judged batch |
