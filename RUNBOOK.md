...
`card:dismiss-local` moves a reviewable card to done with no queued work, no `action:verify`, and no
connector call; it is a Tend-only disposition reversible with `card:return-to-review`. This is
distinct from `card:dismiss`, which queues a `default_cleanup` work item that Codex claims, verifies,
and drains against the source connector.
