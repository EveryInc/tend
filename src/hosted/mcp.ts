...
export function createAttentionMcpServer(env: HostedEnv, session: HostedSession): McpServer {
  ...
  server.registerPrompt("setup_feed_runner", {
    ...
    messages: [{
      ...
      content: {
        type: "text",
        text: `Set up this Codex thread as the local runner for Attention feed ${feedId}. Use threadId ${threadId}. This thread should own exactly this feed; when connecting another feed, use a separate Codex thread and a separate same-thread heartbeat. Bind the feed with bind_feed_thread, inspect the feed setup/state, and create or update one heartbeat automation on this same thread with cadence "${cadence || "every 30 minutes"}". The automation should confirm binding, list queued work first, claim a small batch before using any local connector for queued instructions, execute allowed claimed work, upsert cards only after the relevant claim is held, and use the canonical card shape: id, title, why, and blocks with stable block ids/types. It should verify approved external mutations immediately before acting and complete, fail, block, retry, or cancel work through Attention MCP as appropriate. For close/ignore/already-handled instructions, it should update the card to status "done" and include response plus done: true in complete_work. When no queued work is being handled, it may refresh configured sources with local connectors, upsert only useful cards with provenance in blocks, record source runs/checkpoints, and record sweep batches when the run set represents the current sweep. If the attention MCP tools are configured but not exposed in this thread after OAuth, refresh OAuth and continue setup from a fresh Codex thread.`,
      },
    }],
  }));
